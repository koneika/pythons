...
def report(message):
    ...
    if int(newTime) < int(time.time()):    
        ...
        bot.send_message(chatLogID, "@koneika336" + " @KGB1986g" + " @yeah_I_man" + " @Dedvar")
        bot.send_message(chatSpace, "Отправлено")
        newTime += 60
        # print(str(int(newTime)) + "==" +str(int(newTime)))
...
#         + " " +str(message.from_user.first_name) + " " +str(message.from_user.last_name) + " " +txt)
    ...
    # # print(NoBanWord[0])
    # # fixbug = len(NoBanWord)

    # banWord = ['хуй', 'ебе', 'ебё', 'сук', 'нах', 'ебу', 'еби', 'еба', 'хуя', 'пиз', 'даун', 'бля', 'долбаёб', 'долбаеб']
    # # stackoverflow = len(banWord) - 4
    # NoBanWord = ['наход', 'тебе', 'тебя']
    # # print(stackoverflow, " ", len(banWord))
    #   # проблема маленькая
    # # print()
    ...
    #                          if NoBanWord[k].upper() in txt2[i].upper():
    #                             ban = False
    #                             print("false")
    #                          else:
    #                             ban = True
